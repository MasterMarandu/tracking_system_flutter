-- ============================================================================
-- SCHEMA COMPLETO — TRACKING / LOGÍSTICA (versión endurecida)
-- Ejecutar en orden. Idempotente donde es posible.
-- ============================================================================
...
-- Helpers de seguridad (se definen ya con search_path)
...
CREATE TABLE core_sucursales (
  ...
  CONSTRAINT chk_sucursales_lat CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
  CONSTRAINT chk_sucursales_lon CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
);
...
CREATE TRIGGER update_sucursales_updated_at BEFORE UPDATE ON core_sucursales
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER trg_set_sucursales_ubicacion BEFORE INSERT OR UPDATE OF latitud, longitud
  ON core_sucursales FOR EACH ROW EXECUTE FUNCTION fn_set_ubicacion_from_latlon();
...
-- Unicidad correcta: global vs por-empresa (NULL no es igual entre sí)
...
CREATE TABLE core_usuarios (
  ...
  email VARCHAR(255) NOT NULL, telefono VARCHAR(20),
  activo BOOLEAN DEFAULT TRUE, ultimo_login TIMESTAMPTZ,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
CREATE TABLE core_configuraciones (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL REFERENCES core_empresas(id) ON DELETE CASCADE,
  clave VARCHAR(100) NOT NULL, valor JSONB NOT NULL, descripcion TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID,
...
CREATE TABLE fleet_dispositivos_gps (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL REFERENCES core_empresas(id) ON DELETE CASCADE,
  imei VARCHAR(50) UNIQUE NOT NULL,   -- UNIQUE global correcto (IMEI físico)
  modelo VARCHAR(100), serial VARCHAR(100),
...
CREATE UNIQUE INDEX uq_vehiculos_empresa_matricula_activa ON fleet_vehiculos(empresa_id, matricula) WHERE deleted_at IS NULL;
...
CREATE TABLE fleet_remolques (
  ...
  tipo VARCHAR(50), capacidad_kg DECIMAL(10,2), capacidad_m3 DECIMAL(10,2),
...
CREATE TABLE fleet_conductores (
  ...
  licencia VARCHAR(50) NOT NULL, tipo_licencia VARCHAR(20),
  vencimiento_licencia DATE, telefono VARCHAR(20), foto TEXT,
...
CREATE TABLE fleet_mantenimientos (
  ...
  costo DECIMAL(12,2), kilometraje INTEGER,
...
CREATE TABLE fleet_checklists (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL REFERENCES core_empresas(id) ON DELETE CASCADE,
  viaje_id UUID,  -- FK diferida más abajo
  vehiculo_id UUID NOT NULL REFERENCES fleet_vehiculos(id),
...
CREATE TABLE fleet_checklists_items (
  ...
  nombre VARCHAR(255) NOT NULL, categoria VARCHAR(100), orden INTEGER DEFAULT 0,
...
CREATE TABLE fleet_checklists_plantillas (
  ...
  empresa_id UUID REFERENCES core_empresas(id) ON DELETE CASCADE,
  tipo VARCHAR(30) NOT NULL CHECK (tipo IN ('pre_viaje','post_viaje','mantenimiento')),
  nombre VARCHAR(255) NOT NULL, categoria VARCHAR(100),
  orden INTEGER DEFAULT 0, es_sistema BOOLEAN DEFAULT FALSE,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
CREATE TABLE fleet_capacidades (
  ...
  espacios_adr INTEGER, tiene_refrigeracion BOOLEAN DEFAULT FALSE,
  temperatura_min DECIMAL(5,2), temperatura_max DECIMAL(5,2), zonas_temperatura INTEGER DEFAULT 1,
  vigente_desde DATE DEFAULT CURRENT_DATE, vigente_hasta DATE, activa BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW()
);
CREATE INDEX idx_capacidades_vehiculo ON fleet_capacidades(vehiculo_id) WHERE activa = TRUE;
CREATE TRIGGER update_capacidades_updated_at BEFORE UPDATE ON fleet_capacidades
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TABLE customers_clientes (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL REFERENCES core_empresas(id) ON DELETE CASCADE,
  nombre VARCHAR(255) NOT NULL, ruc VARCHAR(20),
  telefono VARCHAR(20), email VARCHAR(255),
...
CREATE TABLE customers_direcciones (
  ...
  direccion TEXT NOT NULL, ciudad VARCHAR(100), provincia VARCHAR(100),
  pais VARCHAR(100) DEFAULT 'Perú', codigo_postal VARCHAR(20),
  latitud DECIMAL(10,8), longitud DECIMAL(11,8), ubicacion GEOGRAPHY(POINT,4326),
  referencia TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID,
  CONSTRAINT chk_direcciones_lat CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
  CONSTRAINT chk_direcciones_lon CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
);
...
CREATE TRIGGER update_direcciones_updated_at BEFORE UPDATE ON customers_direcciones
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER trg_set_direcciones_ubicacion BEFORE INSERT OR UPDATE OF latitud, longitud
  ON customers_direcciones FOR EACH ROW EXECUTE FUNCTION fn_set_ubicacion_from_latlon();
...
CREATE TABLE customers_remitentes (
  ...
  nombre VARCHAR(255) NOT NULL, documento VARCHAR(20),
  telefono VARCHAR(20), email VARCHAR(255),
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
CREATE TABLE operations_geocercas (
  ...
  radio INTEGER, poligono GEOMETRY(POLYGON,4326), centro GEOGRAPHY(POINT,4326),
  color VARCHAR(20), activa BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID,
...
CREATE TABLE operations_rutas (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL REFERENCES core_empresas(id) ON DELETE CASCADE,
  codigo VARCHAR(50), nombre VARCHAR(255) NOT NULL,
  origen VARCHAR(255), destino VARCHAR(255),
  distancia_km DECIMAL(10,2), tiempo_estimado_min INTEGER, activa BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
CREATE INDEX idx_rutas_empresa ON operations_rutas(empresa_id) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX uq_rutas_empresa_codigo_activa ON operations_rutas(empresa_id, codigo)
...
CREATE TABLE operations_rutas_optimizadas (
  ...
  fecha_calculo TIMESTAMPTZ DEFAULT NOW(), es_activa BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
CREATE TABLE operations_paradas (
  ...
  ruta_id UUID NOT NULL REFERENCES operations_rutas(id) ON DELETE CASCADE,
  direccion_id UUID REFERENCES customers_direcciones(id),
  orden INTEGER NOT NULL, nombre VARCHAR(255), direccion TEXT,
  latitud DECIMAL(10,8), longitud DECIMAL(11,8), ubicacion GEOGRAPHY(POINT,4326),
  tipo VARCHAR(50) CHECK (tipo IN ('recogida','entrega','descanso','combustible','otra')),
  eta_minutos INTEGER, tiempo_estancia_min INTEGER,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID,
  CONSTRAINT chk_paradas_lat CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
  CONSTRAINT chk_paradas_lon CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
);
...
CREATE TRIGGER update_paradas_updated_at BEFORE UPDATE ON operations_paradas
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER trg_set_paradas_ubicacion BEFORE INSERT OR UPDATE OF latitud, longitud
  ON operations_paradas FOR EACH ROW EXECUTE FUNCTION fn_set_ubicacion_from_latlon();

CREATE TABLE operations_eta (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL REFERENCES core_empresas(id) ON DELETE CASCADE,
  viaje_id UUID NOT NULL,  -- FK diferida
  ...
  retraso_min INTEGER DEFAULT 0, distancia_restante_km DECIMAL(10,2),
...
CREATE TABLE operations_viajes (
  ...
  estado VARCHAR(30) DEFAULT 'programado' CHECK (estado IN ('programado','en_curso','pausado','completado','cancelado')),
  ...
  peajes DECIMAL(12,2), costo_total DECIMAL(12,2), observaciones TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
CREATE INDEX idx_viajes_empresa ON operations_viajes(empresa_id) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX uq_viajes_empresa_codigo_activo ON operations_viajes(empresa_id, codigo) WHERE deleted_at IS NULL;
...
-- FKs diferidas hacia viajes
ALTER TABLE fleet_checklists ADD CONSTRAINT fk_checklists_viaje
  FOREIGN KEY (viaje_id) REFERENCES operations_viajes(id);
ALTER TABLE operations_eta ADD CONSTRAINT fk_eta_viaje
  FOREIGN KEY (viaje_id) REFERENCES operations_viajes(id) ON DELETE CASCADE;
...
CREATE TABLE operations_checkpoints (
  ...
  estado VARCHAR(30) DEFAULT 'pendiente' CHECK (estado IN ('pendiente','llego','en_proceso','completado','omitido')),
  latitud DECIMAL(10,8), longitud DECIMAL(11,8), ubicacion GEOGRAPHY(POINT,4326),
  observaciones TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID,
  CONSTRAINT chk_checkpoints_lat CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
  CONSTRAINT chk_checkpoints_lon CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
);
...
CREATE TRIGGER update_checkpoints_updated_at BEFORE UPDATE ON operations_checkpoints
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER trg_set_checkpoints_ubicacion BEFORE INSERT OR UPDATE OF latitud, longitud
  ON operations_checkpoints FOR EACH ROW EXECUTE FUNCTION fn_set_ubicacion_from_latlon();
...
CREATE TABLE operations_viajes_conductores (
  ...
  estado VARCHAR(20) DEFAULT 'asignado' CHECK (estado IN ('asignado','aceptado','en_curso','completado','rechazado','cancelado')),
  fecha_asignacion TIMESTAMPTZ DEFAULT NOW(), fecha_aceptacion TIMESTAMPTZ,
  fecha_inicio TIMESTAMPTZ, fecha_fin TIMESTAMPTZ, observaciones TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
CREATE TABLE operations_viajes_paquetes (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  viaje_id UUID NOT NULL REFERENCES operations_viajes(id) ON DELETE CASCADE,
  paquete_id UUID NOT NULL,  -- FK diferida
  parada_id UUID REFERENCES operations_paradas(id),
  orden_entrega INTEGER,
  estado VARCHAR(30) DEFAULT 'asignado' CHECK (estado IN ('asignado','cargado','en_transito','descargado','entregado','reasignado')),
  hora_asignacion TIMESTAMPTZ DEFAULT NOW(), hora_carga TIMESTAMPTZ,
  hora_descarga TIMESTAMPTZ, hora_entrega TIMESTAMPTZ,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
-- Un paquete solo puede tener UNA asignación activa (corrige bug del índice original)
...
CREATE TABLE operations_asignaciones (
  ...
  referencia_tipo VARCHAR(50), referencia_id UUID, observacion TEXT, metadata JSONB,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
CREATE TABLE shipping_estados_envio (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  codigo VARCHAR(50) UNIQUE NOT NULL, nombre VARCHAR(100) NOT NULL,
  descripcion TEXT, color VARCHAR(20), orden INTEGER, es_final BOOLEAN DEFAULT FALSE,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
CREATE TABLE shipping_tipos_paquete (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID REFERENCES core_empresas(id) ON DELETE CASCADE,
  codigo VARCHAR(50) NOT NULL, nombre VARCHAR(100) NOT NULL,
  descripcion TEXT, es_sistema BOOLEAN DEFAULT FALSE,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
CREATE UNIQUE INDEX uq_tipos_paquete_sistema_codigo ON shipping_tipos_paquete(codigo)
  WHERE empresa_id IS NULL AND deleted_at IS NULL;
CREATE UNIQUE INDEX uq_tipos_paquete_empresa_codigo ON shipping_tipos_paquete(empresa_id, codigo)
  WHERE empresa_id IS NOT NULL AND deleted_at IS NULL;
...
CREATE TABLE shipping_envios (
  ...
  codigo VARCHAR(50) NOT NULL, referencia_cliente VARCHAR(100),
  ...
  estado VARCHAR(30) DEFAULT 'creado' CHECK (estado IN ('creado','preparando','despachado','en_ruta','entregado','cancelado')),
  fecha_programada TIMESTAMPTZ, fecha_despacho TIMESTAMPTZ, fecha_entrega TIMESTAMPTZ,
  observaciones TEXT, valor_total DECIMAL(12,2), costo_total DECIMAL(12,2),
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
CREATE INDEX idx_envios_empresa ON shipping_envios(empresa_id) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX uq_envios_empresa_codigo_activo ON shipping_envios(empresa_id, codigo) WHERE deleted_at IS NULL;
...
CREATE TABLE shipping_paquetes (
  ...
  codigo_qr TEXT, codigo_barras VARCHAR(100),
  cliente_id UUID NOT NULL REFERENCES customers_clientes(id),
  remitente_id UUID REFERENCES customers_remitentes(id),
  destinatario_id UUID REFERENCES customers_destinatarios(id),
  ...
  tipo VARCHAR(50) DEFAULT 'paquete', tipo_id UUID REFERENCES shipping_tipos_paquete(id),
  ...
  mercancia_peligrosa BOOLEAN DEFAULT FALSE, imo_class VARCHAR(20), un_number VARCHAR(30),
  numero_sello VARCHAR(50), requiere_firma BOOLEAN DEFAULT TRUE,
  requiere_otp BOOLEAN DEFAULT FALSE, requiere_documento BOOLEAN DEFAULT FALSE,
  custodia BOOLEAN DEFAULT FALSE, codigo_cliente VARCHAR(100),
...
-- FK diferida viajes_paquetes -> paquetes
ALTER TABLE operations_viajes_paquetes ADD CONSTRAINT fk_viajes_paquetes_paquete
  FOREIGN KEY (paquete_id) REFERENCES shipping_paquetes(id) ON DELETE CASCADE;

CREATE TABLE shipping_cargas (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL REFERENCES core_empresas(id) ON DELETE CASCADE,
  viaje_id UUID REFERENCES operations_viajes(id),
  codigo VARCHAR(50) NOT NULL, descripcion TEXT,
  peso_total DECIMAL(10,2), volumen_total DECIMAL(10,2), cantidad_paquetes INTEGER DEFAULT 0,
  estado VARCHAR(30) DEFAULT 'creada' CHECK (estado IN ('creada','cargando','completa','en_transito','descargada','cancelada')),
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
CREATE INDEX idx_cargas_empresa ON shipping_cargas(empresa_id) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX uq_cargas_empresa_codigo_activa ON shipping_cargas(empresa_id, codigo) WHERE deleted_at IS NULL;
...
CREATE TABLE shipping_paquetes_cargas (
  ...
  orden_carga INTEGER, fecha_asignacion TIMESTAMPTZ DEFAULT NOW(),
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID,
...
CREATE TABLE shipping_carga_evidencias (
  ...
  carga_id UUID NOT NULL REFERENCES shipping_cargas(id) ON DELETE CASCADE,
  viaje_id UUID REFERENCES operations_viajes(id),
  tipo VARCHAR(50) NOT NULL CHECK (tipo IN ('camion_cargado','pallet','sello','documento','photo_galga','otra')),
  url TEXT NOT NULL, descripcion TEXT, numero_sello VARCHAR(50),
  usuario_id UUID REFERENCES core_usuarios(id), fecha TIMESTAMPTZ DEFAULT NOW(),
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
CREATE TABLE shipping_historial_estados (
  ...
  CONSTRAINT chk_historial_lat CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
  CONSTRAINT chk_historial_lon CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
) PARTITION BY RANGE (fecha);

CREATE TABLE shipping_historial_estados_2026_01 PARTITION OF shipping_historial_estados FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
CREATE TABLE shipping_historial_estados_2026_02 PARTITION OF shipping_historial_estados FOR VALUES FROM ('2026-02-01') TO ('2026-03-01');
CREATE TABLE shipping_historial_estados_2026_03 PARTITION OF shipping_historial_estados FOR VALUES FROM ('2026-03-01') TO ('2026-04-01');
CREATE TABLE shipping_historial_estados_2026_04 PARTITION OF shipping_historial_estados FOR VALUES FROM ('2026-04-01') TO ('2026-05-01');
CREATE TABLE shipping_historial_estados_2026_05 PARTITION OF shipping_historial_estados FOR VALUES FROM ('2026-05-01') TO ('2026-06-01');
CREATE TABLE shipping_historial_estados_2026_06 PARTITION OF shipping_historial_estados FOR VALUES FROM ('2026-06-01') TO ('2026-07-01');
CREATE TABLE shipping_historial_estados_2026_07 PARTITION OF shipping_historial_estados FOR VALUES FROM ('2026-07-01') TO ('2026-08-01');
CREATE TABLE shipping_historial_estados_2026_08 PARTITION OF shipping_historial_estados FOR VALUES FROM ('2026-08-01') TO ('2026-09-01');
CREATE TABLE shipping_historial_estados_2026_09 PARTITION OF shipping_historial_estados FOR VALUES FROM ('2026-09-01') TO ('2026-10-01');
CREATE TABLE shipping_historial_estados_2026_10 PARTITION OF shipping_historial_estados FOR VALUES FROM ('2026-10-01') TO ('2026-11-01');
CREATE TABLE shipping_historial_estados_2026_11 PARTITION OF shipping_historial_estados FOR VALUES FROM ('2026-11-01') TO ('2026-12-01');
CREATE TABLE shipping_historial_estados_2026_12 PARTITION OF shipping_historial_estados FOR VALUES FROM ('2026-12-01') TO ('2027-01-01');
...
CREATE TRIGGER trg_set_historial_ubicacion BEFORE INSERT OR UPDATE OF latitud, longitud
  ON shipping_historial_estados FOR EACH ROW EXECUTE FUNCTION fn_set_ubicacion_from_latlon();
...
CREATE TABLE tracking_gps (
  id UUID DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL, viaje_id UUID, vehiculo_id UUID,
  conductor_id UUID, dispositivo_id UUID,
  latitud DECIMAL(10,8) NOT NULL, longitud DECIMAL(11,8) NOT NULL,
  ubicacion GEOGRAPHY(POINT,4326) NOT NULL,
  precision_m DECIMAL(5,2), altitud DECIMAL(8,2), velocidad_kmh DECIMAL(6,2),
  rumbo DECIMAL(5,2), bateria INTEGER CHECK (bateria BETWEEN 0 AND 100),
  internet BOOLEAN, gps BOOLEAN, satelites INTEGER,
  created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,
  PRIMARY KEY (id, created_at),
  CONSTRAINT chk_gps_lat CHECK (latitud BETWEEN -90 AND 90),
  CONSTRAINT chk_gps_lon CHECK (longitud BETWEEN -180 AND 180)
) PARTITION BY RANGE (created_at);

CREATE TABLE tracking_gps_2026_01 PARTITION OF tracking_gps FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
CREATE TABLE tracking_gps_2026_02 PARTITION OF tracking_gps FOR VALUES FROM ('2026-02-01') TO ('2026-03-01');
CREATE TABLE tracking_gps_2026_03 PARTITION OF tracking_gps FOR VALUES FROM ('2026-03-01') TO ('2026-04-01');
CREATE TABLE tracking_gps_2026_04 PARTITION OF tracking_gps FOR VALUES FROM ('2026-04-01') TO ('2026-05-01');
CREATE TABLE tracking_gps_2026_05 PARTITION OF tracking_gps FOR VALUES FROM ('2026-05-01') TO ('2026-06-01');
CREATE TABLE tracking_gps_2026_06 PARTITION OF tracking_gps FOR VALUES FROM ('2026-06-01') TO ('2026-07-01');
CREATE TABLE tracking_gps_2026_07 PARTITION OF tracking_gps FOR VALUES FROM ('2026-07-01') TO ('2026-08-01');
CREATE TABLE tracking_gps_2026_08 PARTITION OF tracking_gps FOR VALUES FROM ('2026-08-01') TO ('2026-09-01');
CREATE TABLE tracking_gps_2026_09 PARTITION OF tracking_gps FOR VALUES FROM ('2026-09-01') TO ('2026-10-01');
CREATE TABLE tracking_gps_2026_10 PARTITION OF tracking_gps FOR VALUES FROM ('2026-10-01') TO ('2026-11-01');
CREATE TABLE tracking_gps_2026_11 PARTITION OF tracking_gps FOR VALUES FROM ('2026-11-01') TO ('2026-12-01');
CREATE TABLE tracking_gps_2026_12 PARTITION OF tracking_gps FOR VALUES FROM ('2026-12-01') TO ('2027-01-01');
...
CREATE TABLE tracking_eventos (
  id UUID DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL, viaje_id UUID, vehiculo_id UUID, conductor_id UUID,
  tipo VARCHAR(50) NOT NULL CHECK (tipo IN (
    'inicio_viaje','fin_viaje','entrada_geocerca','salida_geocerca','detenido',
    'exceso_velocidad','frenada_brusca','aceleracion_brusca','desvio_ruta',
    'incidente','entrega','recogida','parada_no_programada')),
  descripcion TEXT, latitud DECIMAL(10,8), longitud DECIMAL(11,8),
  ubicacion GEOGRAPHY(POINT,4326), metadata JSONB,
  created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,
  PRIMARY KEY (id, created_at),
  CONSTRAINT chk_eventos_lat CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
  CONSTRAINT chk_eventos_lon CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
) PARTITION BY RANGE (created_at);

CREATE TABLE tracking_eventos_2026_01 PARTITION OF tracking_eventos FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
CREATE TABLE tracking_eventos_2026_02 PARTITION OF tracking_eventos FOR VALUES FROM ('2026-02-01') TO ('2026-03-01');
CREATE TABLE tracking_eventos_2026_03 PARTITION OF tracking_eventos FOR VALUES FROM ('2026-03-01') TO ('2026-04-01');
CREATE TABLE tracking_eventos_2026_04 PARTITION OF tracking_eventos FOR VALUES FROM ('2026-04-01') TO ('2026-05-01');
CREATE TABLE tracking_eventos_2026_05 PARTITION OF tracking_eventos FOR VALUES FROM ('2026-05-01') TO ('2026-06-01');
CREATE TABLE tracking_eventos_2026_06 PARTITION OF tracking_eventos FOR VALUES FROM ('2026-06-01') TO ('2026-07-01');
CREATE TABLE tracking_eventos_2026_07 PARTITION OF tracking_eventos FOR VALUES FROM ('2026-07-01') TO ('2026-08-01');
CREATE TABLE tracking_eventos_2026_08 PARTITION OF tracking_eventos FOR VALUES FROM ('2026-08-01') TO ('2026-09-01');
CREATE TABLE tracking_eventos_2026_09 PARTITION OF tracking_eventos FOR VALUES FROM ('2026-09-01') TO ('2026-10-01');
CREATE TABLE tracking_eventos_2026_10 PARTITION OF tracking_eventos FOR VALUES FROM ('2026-10-01') TO ('2026-11-01');
CREATE TABLE tracking_eventos_2026_11 PARTITION OF tracking_eventos FOR VALUES FROM ('2026-11-01') TO ('2026-12-01');
CREATE TABLE tracking_eventos_2026_12 PARTITION OF tracking_eventos FOR VALUES FROM ('2026-12-01') TO ('2027-01-01');
...
CREATE TRIGGER trg_set_eventos_ubicacion BEFORE INSERT OR UPDATE OF latitud, longitud
  ON tracking_eventos FOR EACH ROW EXECUTE FUNCTION fn_set_ubicacion_from_latlon();
...
CREATE TABLE tracking_alertas (
  ...
  titulo VARCHAR(255) NOT NULL, mensaje TEXT, metadata JSONB,
  leido BOOLEAN DEFAULT FALSE, leido_por UUID REFERENCES core_usuarios(id), fecha_leido TIMESTAMPTZ,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
CREATE TABLE tracking_sesiones (
  ...
  distancia_km DECIMAL(10,2), puntos_gps INTEGER DEFAULT 0,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
CREATE TABLE tracking_ultima_posicion (
  ...
  empresa_id UUID NOT NULL, viaje_id UUID, conductor_id UUID, dispositivo_id UUID,
  latitud DECIMAL(10,8) NOT NULL, longitud DECIMAL(11,8) NOT NULL,
  ubicacion GEOGRAPHY(POINT,4326) NOT NULL,
...
CREATE TABLE delivery_firmas (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL REFERENCES core_empresas(id) ON DELETE CASCADE,
  imagen_url TEXT NOT NULL, formato VARCHAR(10) DEFAULT 'png', tamano_bytes INTEGER,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
CREATE TABLE delivery_entregas (
  ...
  viaje_id UUID REFERENCES operations_viajes(id),
  receptor_nombre VARCHAR(255) NOT NULL, receptor_documento VARCHAR(20), receptor_relacion VARCHAR(50),
  firma_id UUID REFERENCES delivery_firmas(id), fecha TIMESTAMPTZ DEFAULT NOW(),
  latitud DECIMAL(10,8), longitud DECIMAL(11,8), ubicacion GEOGRAPHY(POINT,4326),
  observacion TEXT,
  tipo_entrega VARCHAR(20) DEFAULT 'normal' CHECK (tipo_entrega IN ('normal','dejado_en_puerta','vecino','punto_acuerdo')),
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID,
  CONSTRAINT chk_entregas_lat CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
  CONSTRAINT chk_entregas_lon CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
);
...
CREATE TRIGGER update_entregas_updated_at BEFORE UPDATE ON delivery_entregas
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER trg_set_entregas_ubicacion BEFORE INSERT OR UPDATE OF latitud, longitud
  ON delivery_entregas FOR EACH ROW EXECUTE FUNCTION fn_set_ubicacion_from_latlon();
...
CREATE TABLE delivery_incidencias (
  ...
  tipo VARCHAR(50) NOT NULL CHECK (tipo IN (
    ...
    'paquete_extraviado','retraso','acceso_restringido','clima_adverso','averia_vehiculo','otra')),
  descripcion TEXT NOT NULL, foto_url TEXT,
  latitud DECIMAL(10,8), longitud DECIMAL(11,8), ubicacion GEOGRAPHY(POINT,4326),
  estado VARCHAR(20) DEFAULT 'abierta' CHECK (estado IN ('abierta','en_proceso','resuelta','cerrada')),
  resuelta_por UUID REFERENCES core_usuarios(id), fecha_resolucion TIMESTAMPTZ, solucion TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID,
...
CREATE TRIGGER update_incidencias_updated_at BEFORE UPDATE ON delivery_incidencias
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER trg_set_incidencias_ubicacion BEFORE INSERT OR UPDATE OF latitud, longitud
  ON delivery_incidencias FOR EACH ROW EXECUTE FUNCTION fn_set_ubicacion_from_latlon();
...
CREATE TABLE delivery_fotografias (
  ...
  descripcion TEXT, latitud DECIMAL(10,8), longitud DECIMAL(11,8), fecha TIMESTAMPTZ DEFAULT NOW(),
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
CREATE TABLE delivery_sesiones (
  ...
  estado VARCHAR(20) NOT NULL DEFAULT 'en_proceso' CHECK (estado IN ('en_proceso','completada','cancelada')),
  client_operation_id UUID,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- ============================================================================
-- 9. MÓDULO: OPERATIONS — NUEVO MODELO (catálogos + visitas/operaciones/ejecuciones/custodia)
-- ============================================================================
CREATE TABLE operations_estados_operacion (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  codigo VARCHAR(50) NOT NULL, nombre VARCHAR(100) NOT NULL, descripcion TEXT,
  es_final BOOLEAN DEFAULT FALSE, es_sistema BOOLEAN DEFAULT FALSE, orden INTEGER DEFAULT 0,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
CREATE UNIQUE INDEX uq_estados_op_codigo ON operations_estados_operacion(codigo) WHERE es_sistema = TRUE;
...
CREATE TABLE operations_tipos_operacion (
  ...
  requiere_evidencia BOOLEAN DEFAULT FALSE, requiere_firma BOOLEAN DEFAULT FALSE,
  es_sistema BOOLEAN DEFAULT FALSE, created_at TIMESTAMPTZ DEFAULT NOW()
);
CREATE UNIQUE INDEX uq_tipos_op_codigo ON operations_tipos_operacion(codigo) WHERE es_sistema = TRUE;
INSERT INTO operations_tipos_operacion (codigo,nombre,inventory_action,requiere_firma,es_sistema) VALUES
  ...
  ('transferir','Transferir','TRANSFER_OUT',FALSE,TRUE),('inspeccionar','Inspeccionar','NONE',FALSE,TRUE),
...
CREATE TABLE operations_motivos (
  ...
  empresa_id UUID REFERENCES core_empresas(id) ON DELETE CASCADE,
  codigo VARCHAR(50) NOT NULL, nombre VARCHAR(100) NOT NULL,
  categoria VARCHAR(50) NOT NULL CHECK (categoria IN ('fallo_entrega','fallo_recogida','incidencia','cancelacion','devolucion')),
  descripcion TEXT, permite_reintento BOOLEAN DEFAULT TRUE, requiere_evidencia BOOLEAN DEFAULT FALSE,
  genera_devolucion BOOLEAN DEFAULT FALSE, es_responsabilidad_cliente BOOLEAN DEFAULT FALSE,
  activo BOOLEAN DEFAULT TRUE, es_sistema BOOLEAN DEFAULT FALSE, created_at TIMESTAMPTZ DEFAULT NOW()
);
CREATE UNIQUE INDEX uq_motivos_sistema_codigo ON operations_motivos(codigo) WHERE empresa_id IS NULL;
CREATE UNIQUE INDEX uq_motivos_empresa_codigo ON operations_motivos(empresa_id, codigo) WHERE empresa_id IS NOT NULL;
...
CREATE TABLE operations_viaje_visitas (
  ...
  estado VARCHAR(30) DEFAULT 'pendiente' CHECK (estado IN ('pendiente','llego','en_proceso','completada','omitida')),
  hora_llegada TIMESTAMPTZ, hora_salida TIMESTAMPTZ,
  latitud DECIMAL(10,8), longitud DECIMAL(11,8), ubicacion GEOGRAPHY(POINT,4326),
  observaciones TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE INDEX idx_visitas_viaje ON operations_viaje_visitas(viaje_id) WHERE deleted_at IS NULL;
CREATE INDEX idx_visitas_orden ON operations_viaje_visitas(viaje_id, orden);
CREATE INDEX idx_visitas_ubicacion ON operations_viaje_visitas USING GIST(ubicacion);
CREATE TRIGGER trg_set_visitas_ubicacion BEFORE INSERT OR UPDATE OF latitud, longitud
  ON operations_viaje_visitas FOR EACH ROW EXECUTE FUNCTION fn_set_ubicacion_from_latlon();
CREATE TRIGGER update_visitas_updated_at BEFORE UPDATE ON operations_viaje_visitas
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TABLE operations_visita_operaciones (
  ...
  total_intentos INTEGER DEFAULT 0, max_intentos INTEGER DEFAULT 3,
  ejecucion_exitosa_id UUID,  -- FK diferida
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
CREATE INDEX idx_operaciones_viaje ON operations_visita_operaciones(viaje_id) WHERE deleted_at IS NULL;
...
CREATE INDEX idx_operaciones_estado ON operations_visita_operaciones(estado_id);
CREATE TRIGGER update_operaciones_updated_at BEFORE UPDATE ON operations_visita_operaciones
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TABLE operations_operacion_ejecuciones (
  ...
  resultado VARCHAR(30) NOT NULL DEFAULT 'en_proceso' CHECK (resultado IN ('en_proceso','exitosa','fallida','parcial','cancelada')),
  motivo_id UUID REFERENCES operations_motivos(id),
  iniciada_en TIMESTAMPTZ NOT NULL DEFAULT NOW(), finalizada_en TIMESTAMPTZ,
  duracion_segundos INTEGER GENERATED ALWAYS AS (
    CASE WHEN finalizada_en IS NOT NULL
      THEN EXTRACT(EPOCH FROM (finalizada_en - iniciada_en))::INTEGER ELSE NULL END) STORED,
  conductor_id UUID REFERENCES fleet_conductores(id),
  usuario_id UUID REFERENCES core_usuarios(id),
  dispositivo_id UUID REFERENCES fleet_dispositivos_gps(id),
  latitud DECIMAL(10,8), longitud DECIMAL(11,8), ubicacion GEOGRAPHY(POINT,4326),
  precision_gps_m DECIMAL(5,2), offline BOOLEAN DEFAULT FALSE, sincronizada_en TIMESTAMPTZ,
  client_operation_id UUID,
  firma_verificada BOOLEAN DEFAULT FALSE, otp_verificado BOOLEAN DEFAULT FALSE, escaneo_verificado BOOLEAN DEFAULT FALSE,
...
CREATE INDEX idx_ejecuciones_operacion ON operations_operacion_ejecuciones(operacion_id);
CREATE INDEX idx_ejecuciones_viaje ON operations_operacion_ejecuciones(viaje_id);
CREATE INDEX idx_ejecuciones_resultado ON operations_operacion_ejecuciones(resultado);
CREATE INDEX idx_ejecuciones_conductor ON operations_operacion_ejecuciones(conductor_id);
CREATE INDEX idx_ejecuciones_fecha ON operations_operacion_ejecuciones(iniciada_en DESC);
CREATE INDEX idx_ejecuciones_offline ON operations_operacion_ejecuciones(offline) WHERE offline = TRUE AND sincronizada_en IS NULL;
CREATE INDEX idx_ejecuciones_ubicacion ON operations_operacion_ejecuciones USING GIST(ubicacion);
CREATE TRIGGER trg_set_ejecuciones_ubicacion BEFORE INSERT OR UPDATE OF latitud, longitud
  ON operations_operacion_ejecuciones FOR EACH ROW EXECUTE FUNCTION fn_set_ubicacion_from_latlon();
CREATE TRIGGER update_ejecuciones_updated_at BEFORE UPDATE ON operations_operacion_ejecuciones
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- FK circular segura
ALTER TABLE operations_visita_operaciones ADD CONSTRAINT fk_op_ejecucion_exitosa
  FOREIGN KEY (ejecucion_exitosa_id) REFERENCES operations_operacion_ejecuciones(id)
  ON DELETE SET NULL DEFERRABLE INITIALLY DEFERRED;

-- ejecuciones append-only
...
BEGIN
  ...
  RETURN NEW;
END; $$;
CREATE TRIGGER trg_prevent_ejecucion_final_mutation BEFORE UPDATE ON operations_operacion_ejecuciones
...
CREATE TABLE operations_evidencias (
  ...
  viaje_id UUID REFERENCES operations_viajes(id),
  paquete_id UUID REFERENCES shipping_paquetes(id),
  tipo VARCHAR(30) NOT NULL CHECK (tipo IN ('foto','firma','audio','video','pdf','scan','documento','otp_log','temperatura','ubicacion')),
  url TEXT, valor JSONB DEFAULT '{}'::jsonb,
  bucket VARCHAR(100), ruta_archivo TEXT, mime_type VARCHAR(100), tamano_bytes BIGINT, hash_sha256 VARCHAR(64),
...
CREATE INDEX idx_evidencias_ubicacion ON operations_evidencias USING GIST(ubicacion);
CREATE TRIGGER trg_set_evidencias_ubicacion BEFORE INSERT OR UPDATE OF latitud, longitud
  ON operations_evidencias FOR EACH ROW EXECUTE FUNCTION fn_set_ubicacion_from_latlon();
...
CREATE TABLE operations_custodia (
  ...
  custodio_tipo VARCHAR(30) NOT NULL CHECK (custodio_tipo IN ('almacen','conductor','hub','cliente','tercero','punto_recogida')),
  custodio_id UUID, custodio_nombre VARCHAR(255),
  viaje_id UUID REFERENCES operations_viajes(id),
  ejecucion_id UUID REFERENCES operations_operacion_ejecuciones(id),
  recibido_en TIMESTAMPTZ NOT NULL DEFAULT NOW(), entregado_en TIMESTAMPTZ,
  latitud DECIMAL(10,8), longitud DECIMAL(11,8), ubicacion GEOGRAPHY(POINT,4326),
  estado_fisico VARCHAR(30) DEFAULT 'integro' CHECK (estado_fisico IN ('integro','dañado','mojado','abierto','incompleto')),
  observaciones TEXT, created_at TIMESTAMPTZ DEFAULT NOW(), created_by UUID,
...
CREATE UNIQUE INDEX uq_custodia_actual_paquete ON operations_custodia(paquete_id) WHERE entregado_en IS NULL;
ALTER TABLE operations_custodia ADD CONSTRAINT ex_custodia_no_overlap
  EXCLUDE USING gist (
    paquete_id WITH =,
...
CREATE TRIGGER trg_set_custodia_ubicacion BEFORE INSERT OR UPDATE OF latitud, longitud
  ON operations_custodia FOR EACH ROW EXECUTE FUNCTION fn_set_ubicacion_from_latlon();

CREATE TABLE operations_restricciones (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL REFERENCES core_empresas(id) ON DELETE CASCADE,
  referencia_tipo VARCHAR(30) NOT NULL CHECK (referencia_tipo IN ('paquete','direccion','cliente','visita','vehiculo')),
  referencia_id UUID NOT NULL,
  tipo VARCHAR(50) NOT NULL CHECK (tipo IN (
    'temperatura','adr','fragil','ventana_horaria','acceso_restringido','requiere_grua',
    'no_escaleras','peso_maximo','tipo_vehiculo','requiere_hidraulica','zona_peaton','altura_maxima')),
  valor JSONB NOT NULL DEFAULT '{}'::jsonb,
  es_dura BOOLEAN DEFAULT TRUE, penalizacion INTEGER DEFAULT 0, activa BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
CREATE INDEX idx_restricciones_referencia ON operations_restricciones(referencia_tipo, referencia_id) WHERE activa = TRUE;
...
CREATE TABLE planning_optimizaciones (
  ...
  algoritmo VARCHAR(50) NOT NULL CHECK (algoritmo IN
    ('manual','google_or_tools','ortools_vrp','vroom','osrm','ia_propietaria','nearest_neighbor','clarke_wright')),
  version_algoritmo VARCHAR(20), version_plan INTEGER NOT NULL DEFAULT 1,
  parametros JSONB, input_snapshot JSONB,
  distancia_total_km DECIMAL(10,2), tiempo_total_min INTEGER, costo_estimado DECIMAL(12,2),
  paradas_reordenadas INTEGER, score_calidad DECIMAL(5,2), mejora_vs_anterior_pct DECIMAL(5,2),
  duracion_computo_ms INTEGER, aplicada BOOLEAN DEFAULT FALSE, aplicada_en TIMESTAMPTZ,
...
-- ============================================================================
-- 11. MÓDULO: AUDIT (particionada)
-- ============================================================================
CREATE TABLE audit_auditoria (
  id UUID DEFAULT uuid_generate_v4(),
  empresa_id UUID, usuario_id UUID, usuario_nombre VARCHAR(255),
  accion VARCHAR(50) NOT NULL CHECK (accion IN ('INSERT','UPDATE','DELETE','LOGIN','LOGOUT','EXPORT','IMPORT')),
  tabla_afectada VARCHAR(100) NOT NULL, registro_id UUID,
  datos_antes JSONB, datos_despues JSONB, ip_address INET, user_agent TEXT, metadata JSONB,
  created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,
  PRIMARY KEY (id, created_at)
) PARTITION BY RANGE (created_at);

CREATE TABLE audit_auditoria_2026_01 PARTITION OF audit_auditoria FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
CREATE TABLE audit_auditoria_2026_02 PARTITION OF audit_auditoria FOR VALUES FROM ('2026-02-01') TO ('2026-03-01');
CREATE TABLE audit_auditoria_2026_03 PARTITION OF audit_auditoria FOR VALUES FROM ('2026-03-01') TO ('2026-04-01');
CREATE TABLE audit_auditoria_2026_04 PARTITION OF audit_auditoria FOR VALUES FROM ('2026-04-01') TO ('2026-05-01');
CREATE TABLE audit_auditoria_2026_05 PARTITION OF audit_auditoria FOR VALUES FROM ('2026-05-01') TO ('2026-06-01');
CREATE TABLE audit_auditoria_2026_06 PARTITION OF audit_auditoria FOR VALUES FROM ('2026-06-01') TO ('2026-07-01');
CREATE TABLE audit_auditoria_2026_07 PARTITION OF audit_auditoria FOR VALUES FROM ('2026-07-01') TO ('2026-08-01');
CREATE TABLE audit_auditoria_2026_08 PARTITION OF audit_auditoria FOR VALUES FROM ('2026-08-01') TO ('2026-09-01');
CREATE TABLE audit_auditoria_2026_09 PARTITION OF audit_auditoria FOR VALUES FROM ('2026-09-01') TO ('2026-10-01');
CREATE TABLE audit_auditoria_2026_10 PARTITION OF audit_auditoria FOR VALUES FROM ('2026-10-01') TO ('2026-11-01');
CREATE TABLE audit_auditoria_2026_11 PARTITION OF audit_auditoria FOR VALUES FROM ('2026-11-01') TO ('2026-12-01');
CREATE TABLE audit_auditoria_2026_12 PARTITION OF audit_auditoria FOR VALUES FROM ('2026-12-01') TO ('2027-01-01');
...
CREATE TABLE storage_documentos (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL REFERENCES core_empresas(id) ON DELETE CASCADE,
  bucket VARCHAR(100) NOT NULL, ruta_archivo TEXT NOT NULL, url_publica TEXT,
  nombre_original VARCHAR(255), mime_type VARCHAR(100), tamano_bytes BIGINT,
  referencia_tipo VARCHAR(50), referencia_id UUID, metadata JSONB,
  created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
  created_by UUID, updated_by UUID, deleted_at TIMESTAMPTZ, deleted_by UUID
);
...
-- ============================================================================
-- 13. MÓDULO: INTEGRATION (outbox / inbox)
-- ============================================================================
CREATE TABLE integration_outbox (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID NOT NULL REFERENCES core_empresas(id) ON DELETE CASCADE,
  aggregate_type VARCHAR(50) NOT NULL, aggregate_id UUID NOT NULL,
  event_type VARCHAR(100) NOT NULL, payload JSONB NOT NULL, destino VARCHAR(50),
  status VARCHAR(20) NOT NULL DEFAULT 'pendiente' CHECK (status IN ('pendiente','publicando','publicado','fallido','descartado')),
  retry_count INTEGER DEFAULT 0, max_retries INTEGER DEFAULT 5,
  next_retry_at TIMESTAMPTZ, last_error TEXT, idempotency_key VARCHAR(255),
  created_at TIMESTAMPTZ DEFAULT NOW(), published_at TIMESTAMPTZ,
  CONSTRAINT uq_outbox_idempotency UNIQUE (idempotency_key)
);
CREATE INDEX idx_outbox_pendientes ON integration_outbox(created_at) WHERE status IN ('pendiente','fallido');
CREATE INDEX idx_outbox_aggregate ON integration_outbox(aggregate_type, aggregate_id);
CREATE INDEX idx_outbox_retry ON integration_outbox(next_retry_at) WHERE status = 'fallido' AND next_retry_at IS NOT NULL;

CREATE TABLE integration_inbox (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  empresa_id UUID REFERENCES core_empresas(id),
  origen VARCHAR(50) NOT NULL, external_id VARCHAR(255) NOT NULL,
  event_type VARCHAR(100) NOT NULL, payload JSONB NOT NULL,
  status VARCHAR(20) DEFAULT 'recibido' CHECK (status IN ('recibido','procesando','procesado','fallido','duplicado')),
...
-- ============================================================================
-- 14. FUNCIONES AUXILIARES DE NEGOCIO
-- ============================================================================
CREATE OR REPLACE FUNCTION generar_tracking_number()
RETURNS TEXT LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE nuevo_tracking TEXT; existe BOOLEAN;
BEGIN
  LOOP
    nuevo_tracking := 'TRK' || TO_CHAR(NOW(),'YYYYMMDD') || LPAD(FLOOR(RANDOM()*1000000)::TEXT,6,'0');
    SELECT EXISTS(SELECT 1 FROM shipping_paquetes WHERE tracking_number = nuevo_tracking) INTO existe;
    EXIT WHEN NOT existe;
  END LOOP;
  RETURN nuevo_tracking;
END; $$;

CREATE OR REPLACE FUNCTION calcular_distancia_km(lat1 DECIMAL, lon1 DECIMAL, lat2 DECIMAL, lon2 DECIMAL)
RETURNS DECIMAL LANGUAGE plpgsql SET search_path = public, pg_temp AS $$
BEGIN
  RETURN ST_Distance(ST_MakePoint(lon1,lat1)::GEOGRAPHY, ST_MakePoint(lon2,lat2)::GEOGRAPHY)/1000;
END; $$;
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_temp AS $$
  ...
  WHERE c.paquete_id = p_paquete_id AND c.recibido_en <= p_momento
    AND (c.entregado_en IS NULL OR c.entregado_en > p_momento)
  ORDER BY c.secuencia DESC LIMIT 1;
$$;

CREATE OR REPLACE FUNCTION generar_codigo_empresa(p_tabla TEXT, p_empresa_id UUID, p_prefijo TEXT DEFAULT '')
RETURNS TEXT LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE v_codigo TEXT; v_existe BOOLEAN; v_counter INTEGER := 1;
BEGIN
  LOOP
...
CREATE OR REPLACE FUNCTION public.publicar_evento_outbox(
  ...
  p_event_type VARCHAR, p_payload JSONB, p_destino VARCHAR DEFAULT NULL, p_idempotency_key VARCHAR DEFAULT NULL)
RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE v_outbox_id UUID; v_idempotency VARCHAR(255);
BEGIN
  v_idempotency := COALESCE(p_idempotency_key,
    p_aggregate_type||':'||p_aggregate_id::text||':'||p_event_type||':'||
    COALESCE(p_payload->>'event_id', p_payload->>'version', encode(digest(p_payload::text,'sha256'),'hex')));
  INSERT INTO integration_outbox (empresa_id,aggregate_type,aggregate_id,event_type,payload,destino,idempotency_key)
  VALUES (p_empresa_id,p_aggregate_type,p_aggregate_id,p_event_type,p_payload,p_destino,v_idempotency)
  ON CONFLICT (idempotency_key) DO NOTHING RETURNING id INTO v_outbox_id;
  IF v_outbox_id IS NULL THEN
    SELECT id INTO v_outbox_id FROM integration_outbox WHERE idempotency_key = v_idempotency;
  END IF;
  RETURN v_outbox_id;
END; $$;

CREATE OR REPLACE FUNCTION public.integration_dequeue_outbox(p_limit INTEGER DEFAULT 100)
RETURNS SETOF integration_outbox LANGUAGE sql SECURITY DEFINER SET search_path = public, pg_temp AS $$
  WITH pendientes AS (
    SELECT id FROM integration_outbox
    WHERE status IN ('pendiente','fallido') AND retry_count < max_retries
      AND (next_retry_at IS NULL OR next_retry_at <= NOW())
    ORDER BY created_at LIMIT p_limit FOR UPDATE SKIP LOCKED
  )
  UPDATE integration_outbox o SET status='publicando'
  FROM pendientes p WHERE o.id = p.id RETURNING o.*;
$$;
...
RETURNS VOID LANGUAGE sql SECURITY DEFINER SET search_path = public, pg_temp AS $$
  ...
  SET status='fallido', retry_count=retry_count+1, last_error=p_error,
...
DECLARE v_empresa_id UUID; v_usuario_id UUID; v_registro_id UUID;
BEGIN
  IF TG_OP = 'INSERT' THEN
    v_empresa_id := NEW.empresa_id; v_usuario_id := NEW.created_by; v_registro_id := NEW.id;
    INSERT INTO audit_auditoria (empresa_id,usuario_id,accion,tabla_afectada,registro_id,datos_antes,datos_despues)
    VALUES (v_empresa_id,v_usuario_id,'INSERT',TG_TABLE_NAME,v_registro_id,NULL,to_jsonb(NEW));
    RETURN NEW;
  ELSIF TG_OP = 'UPDATE' THEN
    v_empresa_id := COALESCE(NEW.empresa_id, OLD.empresa_id);
    v_usuario_id := COALESCE(NEW.updated_by, OLD.updated_by); v_registro_id := NEW.id;
    INSERT INTO audit_auditoria (empresa_id,usuario_id,accion,tabla_afectada,registro_id,datos_antes,datos_despues)
    VALUES (v_empresa_id,v_usuario_id,'UPDATE',TG_TABLE_NAME,v_registro_id,to_jsonb(OLD),to_jsonb(NEW));
    RETURN NEW;
  ELSIF TG_OP = 'DELETE' THEN
    v_empresa_id := OLD.empresa_id; v_usuario_id := OLD.updated_by; v_registro_id := OLD.id;
    INSERT INTO audit_auditoria (empresa_id,usuario_id,accion,tabla_afectada,registro_id,datos_antes,datos_despues)
    VALUES (v_empresa_id,v_usuario_id,'DELETE',TG_TABLE_NAME,v_registro_id,to_jsonb(OLD),NULL);
    RETURN OLD;
  END IF;
  RETURN NULL;
END; $$;

CREATE TRIGGER trg_auditoria_paquetes AFTER INSERT OR UPDATE OR DELETE ON shipping_paquetes
  FOR EACH ROW EXECUTE FUNCTION fn_auditoria_general();
CREATE TRIGGER trg_auditoria_viajes AFTER INSERT OR UPDATE OR DELETE ON operations_viajes
  FOR EACH ROW EXECUTE FUNCTION fn_auditoria_general();
CREATE TRIGGER trg_auditoria_entregas AFTER INSERT OR UPDATE OR DELETE ON delivery_entregas
  FOR EACH ROW EXECUTE FUNCTION fn_auditoria_general();
CREATE TRIGGER trg_auditoria_envios AFTER INSERT OR UPDATE OR DELETE ON shipping_envios
  FOR EACH ROW EXECUTE FUNCTION fn_auditoria_general();
CREATE TRIGGER trg_auditoria_viajes_paquetes AFTER INSERT OR UPDATE OR DELETE ON operations_viajes_paquetes
  FOR EACH ROW EXECUTE FUNCTION fn_auditoria_general();
CREATE TRIGGER trg_auditoria_asignaciones AFTER INSERT OR UPDATE OR DELETE ON operations_asignaciones
  FOR EACH ROW EXECUTE FUNCTION fn_auditoria_general();
CREATE TRIGGER trg_auditoria_tipos_paquete AFTER INSERT OR UPDATE OR DELETE ON shipping_tipos_paquete
  FOR EACH ROW EXECUTE FUNCTION fn_auditoria_general();

-- ============================================================================
-- 16. TRIGGER: última posición GPS
-- ============================================================================
...
BEGIN
  IF NEW.vehiculo_id IS NOT NULL THEN
    INSERT INTO tracking_ultima_posicion (
      vehiculo_id,empresa_id,viaje_id,conductor_id,dispositivo_id,latitud,longitud,ubicacion,
      precision_m,velocidad_kmh,rumbo,bateria,internet,gps,satelites,created_at,updated_at
    ) VALUES (
      NEW.vehiculo_id,NEW.empresa_id,NEW.viaje_id,NEW.conductor_id,NEW.dispositivo_id,NEW.latitud,NEW.longitud,NEW.ubicacion,
      NEW.precision_m,NEW.velocidad_kmh,NEW.rumbo,NEW.bateria,NEW.internet,NEW.gps,NEW.satelites,NEW.created_at,NOW()
    )
    ON CONFLICT (vehiculo_id) DO UPDATE SET
      viaje_id=EXCLUDED.viaje_id, conductor_id=EXCLUDED.conductor_id, dispositivo_id=EXCLUDED.dispositivo_id,
      latitud=EXCLUDED.latitud, longitud=EXCLUDED.longitud, ubicacion=EXCLUDED.ubicacion,
      precision_m=EXCLUDED.precision_m, velocidad_kmh=EXCLUDED.velocidad_kmh, rumbo=EXCLUDED.rumbo,
      bateria=EXCLUDED.bateria, internet=EXCLUDED.internet, gps=EXCLUDED.gps, satelites=EXCLUDED.satelites,
      created_at=EXCLUDED.created_at, updated_at=NOW();
  END IF;
  RETURN NEW;
END; $$;
CREATE TRIGGER trg_update_ultima_posicion AFTER INSERT ON tracking_gps
  FOR EACH ROW EXECUTE FUNCTION fn_update_ultima_posicion();

-- ============================================================================
-- 17. TRIGGER: auto-generar visitas + operaciones al asignar ruta al viaje
-- ============================================================================
CREATE OR REPLACE FUNCTION public.fn_generar_visitas_operaciones()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE v_tipo_entregar_id UUID; v_estado_pendiente_id UUID;
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM operations_viaje_visitas WHERE viaje_id = NEW.id AND deleted_at IS NULL) THEN
    RETURN NEW;
  END IF;
  SELECT id INTO v_tipo_entregar_id FROM operations_tipos_operacion WHERE codigo='entregar' AND es_sistema=TRUE;
  SELECT id INTO v_estado_pendiente_id FROM operations_estados_operacion WHERE codigo='pendiente' AND es_sistema=TRUE;
...
CREATE TRIGGER trg_generar_visitas_operaciones
  ...
  FOR EACH ROW WHEN (NEW.ruta_id IS NOT NULL AND NEW.deleted_at IS NULL)
  EXECUTE FUNCTION public.fn_generar_visitas_operaciones();

-- ============================================================================
-- 18. FUNCIÓN NÚCLEO: registrar_ejecucion
-- ============================================================================
CREATE OR REPLACE FUNCTION public.registrar_ejecucion(
  p_operacion_id UUID, p_resultado VARCHAR, p_conductor_id UUID DEFAULT NULL,
  p_motivo_codigo VARCHAR DEFAULT NULL, p_latitud DECIMAL DEFAULT NULL, p_longitud DECIMAL DEFAULT NULL,
  p_receptor_nombre VARCHAR DEFAULT NULL, p_client_operation_id UUID DEFAULT NULL, p_offline BOOLEAN DEFAULT FALSE)
RETURNS TABLE (success BOOLEAN, ejecucion_id UUID, operacion_completada BOOLEAN, intentos_restantes INTEGER, message TEXT)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE
  v_op RECORD; v_ejecucion_id UUID; v_numero_intento INTEGER; v_motivo_id UUID; v_empresa_id UUID;
  v_estado_completada_id UUID; v_estado_fallida_id UUID; v_intentos_restantes INTEGER;
BEGIN
  IF p_client_operation_id IS NOT NULL THEN
    SELECT id INTO v_ejecucion_id FROM operations_operacion_ejecuciones WHERE client_operation_id = p_client_operation_id;
    IF v_ejecucion_id IS NOT NULL THEN
      RETURN QUERY SELECT TRUE, v_ejecucion_id, FALSE, 0, 'Ejecución ya registrada (idempotente)'::TEXT; RETURN;
    END IF;
  END IF;

  SELECT * INTO v_op FROM operations_visita_operaciones WHERE id = p_operacion_id AND deleted_at IS NULL FOR UPDATE;
  IF NOT FOUND THEN RETURN QUERY SELECT FALSE, NULL::UUID, FALSE, 0, 'Operación no encontrada'::TEXT; RETURN; END IF;
  ...
  INSERT INTO operations_operacion_ejecuciones (
    empresa_id,operacion_id,viaje_id,visita_id,paquete_id,numero_intento,resultado,motivo_id,
    conductor_id,latitud,longitud,receptor_nombre,client_operation_id,offline,finalizada_en,sincronizada_en,created_by
  ) VALUES (
    v_empresa_id,p_operacion_id,v_op.viaje_id,v_op.visita_id,v_op.paquete_id,v_numero_intento,p_resultado,v_motivo_id,
    p_conductor_id,p_latitud,p_longitud,p_receptor_nombre,p_client_operation_id,p_offline,NOW(),
  ...
  UPDATE operations_visita_operaciones SET total_intentos = v_numero_intento, updated_at = NOW() WHERE id = p_operacion_id;

  IF p_resultado = 'exitosa' THEN
    SELECT id INTO v_estado_completada_id FROM operations_estados_operacion WHERE codigo='completada' AND es_sistema=TRUE;
    UPDATE operations_visita_operaciones
    SET estado_id = v_estado_completada_id, ejecucion_exitosa_id = v_ejecucion_id, updated_at = NOW()
    ...
    PERFORM public.publicar_evento_outbox(
      ...
      jsonb_build_object('operacion_id',p_operacion_id,'ejecucion_id',v_ejecucion_id,'paquete_id',v_op.paquete_id,'viaje_id',v_op.viaje_id),
      NULL,'paquete:'||v_op.paquete_id::text||':operacion_completada:'||v_ejecucion_id::text);
  ELSIF v_numero_intento >= v_op.max_intentos AND p_resultado = 'fallida' THEN
    SELECT id INTO v_estado_fallida_id FROM operations_estados_operacion WHERE codigo='fallida' AND es_sistema=TRUE;
    UPDATE operations_visita_operaciones SET estado_id = v_estado_fallida_id, updated_at = NOW() WHERE id = p_operacion_id;
  END IF;

  v_intentos_restantes := GREATEST(v_op.max_intentos - v_numero_intento, 0);
  RETURN QUERY SELECT TRUE, v_ejecucion_id, (p_resultado='exitosa'), v_intentos_restantes,
    CASE WHEN p_resultado='exitosa' THEN 'Operación completada exitosamente'
         ELSE 'Intento registrado. Reintentos restantes: '||v_intentos_restantes END::TEXT;
...
-- ============================================================================
-- 19. FUNCIONES DE REGISTRO (empresa/usuario, conductor, geocerca)
-- ============================================================================
...
BEGIN
  ...
  END IF;
  SELECT id INTO v_rol_id FROM core_roles WHERE nombre = p_rol_nombre AND es_sistema = TRUE LIMIT 1;
  IF p_mode = 'new_company' THEN
    ...
    END IF;
    INSERT INTO core_empresas (nombre,ruc,email,estado) VALUES (p_company_name,p_company_ruc,p_email,'activo')
...
CREATE OR REPLACE FUNCTION public.registrar_conductor(
  p_empresa_id UUID, p_nombre VARCHAR, p_apellido VARCHAR, p_email VARCHAR, p_password VARCHAR,
  p_licencia VARCHAR, p_telefono VARCHAR DEFAULT NULL, p_tipo_licencia VARCHAR DEFAULT NULL,
  p_vencimiento_licencia DATE DEFAULT NULL, p_auth_user_id UUID DEFAULT NULL)
RETURNS TABLE (success BOOLEAN, conductor_id UUID, usuario_id UUID, auth_user_id UUID, message TEXT)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE v_conductor_id UUID; v_usuario_id UUID; v_rol_id UUID; v_existe_conductor BOOLEAN; v_existe_usuario BOOLEAN;
BEGIN
  IF auth.uid() IS NOT NULL AND public.user_empresa_id() IS DISTINCT FROM p_empresa_id THEN
    RETURN QUERY SELECT FALSE,NULL::UUID,NULL::UUID,NULL::UUID,'No autorizado para registrar conductores en esta empresa'::TEXT; RETURN;
  END IF;
  ...
  END IF;
  SELECT EXISTS(SELECT 1 FROM fleet_conductores WHERE empresa_id=p_empresa_id AND licencia=p_licencia AND deleted_at IS NULL) INTO v_existe_conductor;
  ...
  SELECT EXISTS(SELECT 1 FROM core_usuarios WHERE email=p_email AND deleted_at IS NULL) INTO v_existe_usuario;
  ...
  IF p_auth_user_id IS NULL THEN
    RETURN QUERY SELECT FALSE,NULL::UUID,NULL::UUID,NULL::UUID,'Se requiere auth_user_id (crear usuario en auth.users primero)'::TEXT; RETURN;
  END IF;
...
CREATE OR REPLACE FUNCTION public.guardar_geocerca(
  p_id UUID DEFAULT NULL, p_empresa_id UUID DEFAULT NULL, p_nombre VARCHAR DEFAULT NULL,
  p_tipo VARCHAR DEFAULT 'circulo', p_latitud DOUBLE PRECISION DEFAULT NULL, p_longitud DOUBLE PRECISION DEFAULT NULL,
  p_radio INTEGER DEFAULT NULL, p_color VARCHAR DEFAULT '#3B82F6', p_activa BOOLEAN DEFAULT TRUE, p_poligono JSONB DEFAULT NULL)
RETURNS TABLE (success BOOLEAN, geocerca_id UUID, message TEXT)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE v_geocerca_id UUID; v_centro GEOGRAPHY; v_poligono_geom GEOMETRY(POLYGON,4326);
BEGIN
