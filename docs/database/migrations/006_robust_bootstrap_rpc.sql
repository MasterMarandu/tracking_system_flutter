-- ============================================================================
-- MIGRATION 006: RPC get_driver_bootstrap robusta
-- ============================================================================
-- Funciona con o sin paradas, con o sin ruta, con o sin vehículo
-- ============================================================================
...
BEGIN
    ...
    -- 4. Get assigned vehicle (opcional)
    ...
    -- 7. Get current stop (primer checkpoint incompleto, robusto)
    IF v_trip_id IS NOT NULL THEN
        SELECT jsonb_build_object(
            'id', COALESCE(op.id, oc.parada_id::TEXT, oc.id::TEXT),
            'checkpoint_id', oc.id,
            'name', COALESCE(op.nombre, 'Sin nombre'),
            'address', COALESCE(op.direccion, 'Sin dirección'),
            ...
            'latitud', COALESCE(op.latitud, oc.latitud, 0),
            'longitud', COALESCE(op.longitud, oc.longitud, 0),
            'distance_km', 0,
        ...
        ORDER BY
            (SELECT orden FROM operations_paradas WHERE id = oc.parada_id) NULLS LAST,
            oc.created_at ASC NULLS LAST
        LIMIT 1;
