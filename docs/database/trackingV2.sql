-- ============================================================================
-- SISTEMA DE TRACKING Y LOGÍSTICA - SCHEMA COMPLETO UNIFICADO
...
CREATE TABLE core_sucursales (
    ...
    deleted_at TIMESTAMP WITH TIME ZONE,
    deleted_by UUID,
    CONSTRAINT chk_sucursales_latitud CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
    CONSTRAINT chk_sucursales_longitud CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
);
...
-- Unicidad correcta para catálogos con empresa_id NULL
CREATE UNIQUE INDEX uq_roles_sistema_nombre
    ON core_roles(nombre)
    WHERE empresa_id IS NULL AND deleted_at IS NULL;
CREATE UNIQUE INDEX uq_roles_empresa_nombre
...
CREATE UNIQUE INDEX uq_vehiculos_empresa_matricula_activa
    ON fleet_vehiculos(empresa_id, matricula) WHERE deleted_at IS NULL;
...
-- Tabla: capacidades multidimensionales
...
-- ============================================================================
-- MÓDULO: OPERATIONS - CATÁLOGOS
-- ============================================================================

-- Catálogo: estados de operación
...
CREATE UNIQUE INDEX uq_estados_op_codigo
...
CREATE UNIQUE INDEX uq_tipos_op_codigo
...
CREATE INDEX idx_motivos_categoria
...
-- ============================================================================
-- MÓDULO: OPERATIONS - ENTIDADES PRINCIPALES
-- ============================================================================
...
CREATE INDEX idx_rutas_opt_activa ON operations_rutas_optimizadas(es_activa)
    WHERE es_activa = TRUE AND deleted_at IS NULL;
...
CREATE TABLE operations_paradas (
    ...
    deleted_at TIMESTAMP WITH TIME ZONE,
    deleted_by UUID,
    CONSTRAINT chk_paradas_latitud CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
    CONSTRAINT chk_paradas_longitud CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
);
...
CREATE UNIQUE INDEX uq_viajes_empresa_codigo_activo
    ON operations_viajes(empresa_id, codigo) WHERE deleted_at IS NULL;
...
-- Tabla: checkpoints (compatibilidad)
...
CREATE TABLE operations_viajes_conductores (
    ...
    estado VARCHAR(20) DEFAULT 'asignado' CHECK (estado IN (
        'asignado', 'aceptado', 'en_curso', 'completado', 'rechazado', 'cancelado'
    )),
...
-- Tabla: viajes_paquetes (puente viaje-paquete, compatibilidad)
...
-- Paquete solo puede tener UNA asignación activa (en cualquier viaje)
...
-- Tabla: restricciones (para VRP solver)
...
-- ============================================================================
-- MÓDULO: OPERATIONS - VISITAS / OPERACIONES / EJECUCIONES
-- ============================================================================

-- Tabla: visitas (parada ejecutable dentro de un viaje)
...
CREATE UNIQUE INDEX uq_tipos_paquete_sistema_codigo
    ON shipping_tipos_paquete(codigo)
    WHERE empresa_id IS NULL AND deleted_at IS NULL;
CREATE UNIQUE INDEX uq_tipos_paquete_empresa_codigo
...
CREATE UNIQUE INDEX uq_envios_empresa_codigo_activo
    ON shipping_envios(empresa_id, codigo) WHERE deleted_at IS NULL;
...
-- FK diferida para viajes_paquetes
ALTER TABLE operations_viajes_paquetes
    ADD CONSTRAINT fk_viajes_paquetes_paquete FOREIGN KEY (paquete_id)
    REFERENCES shipping_paquetes(id) ON DELETE CASCADE;

-- Tabla: operaciones (el "QUÉ" - inmutable en intención)
...
-- Tabla: ejecuciones (el "CÓMO" - 0..N intentos)
...
CREATE INDEX idx_ejecuciones_offline ON operations_operacion_ejecuciones(offline)
...
-- Ejecuciones append-only (trigger anti-mutación tras cierre)
...
CREATE UNIQUE INDEX uq_cargas_empresa_codigo_activa
    ON shipping_cargas(empresa_id, codigo) WHERE deleted_at IS NULL;

CREATE TRIGGER update_cargas_updated_at BEFORE UPDATE ON shipping_cargas
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Tabla: paquetes_cargas
...
-- Tabla: historial_estados (PARTICIONADA)
CREATE TABLE shipping_historial_estados (
    ...
    CONSTRAINT chk_historial_latitud CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
    CONSTRAINT chk_historial_longitud CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
...
-- Tabla: tracking_gps (PARTICIONADA)
...
CREATE TABLE tracking_eventos (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL,
    PRIMARY KEY (id, created_at),
    CONSTRAINT chk_eventos_latitud CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
    CONSTRAINT chk_eventos_longitud CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
) PARTITION BY RANGE (created_at);
...
-- Tabla: última posición GPS (cache materializada)
...
-- Trigger para mantener última posición GPS
...
-- ============================================================================
-- MÓDULO: DELIVERY (compatibilidad)
-- ============================================================================
...
CREATE TABLE delivery_entregas (
    ...
    deleted_at TIMESTAMP WITH TIME ZONE,
    deleted_by UUID,
    CONSTRAINT chk_entregas_latitud CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
    CONSTRAINT chk_entregas_longitud CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
);
...
-- ============================================================================
-- MÓDULO: INTEGRATION (Outbox/Inbox)
-- ============================================================================
...
-- Función: generar código por empresa
...
-- Función: custodia en un momento dado
...
-- Función: publicar evento outbox con idempotencia
...
-- Función: dequeue outbox (para workers)
...
-- Función: marcar fallido con retry exponencial
...
-- ============================================================================
-- FUNCIÓN NÚCLEO: registrar_ejecucion
-- ============================================================================
...
BEGIN
    ...
    -- 4) Si ya está completada, no más intentos
...
BEGIN
    IF p_auth_user_id IS NULL OR p_email IS NULL OR p_nombre IS NULL OR p_apellido IS NULL THEN
        RETURN QUERY SELECT FALSE, NULL::UUID, NULL::UUID, 'Faltan campos obligatorios'::TEXT;
        RETURN;
    END IF;
    ...
    IF p_mode = 'new_company' THEN
        IF p_company_name IS NULL THEN
            RETURN QUERY SELECT FALSE, NULL::UUID, NULL::UUID, 'Nombre de empresa es obligatorio'::TEXT;
            RETURN;
        END IF;
    ...
    ELSIF p_mode = 'join_company' THEN
        IF p_invite_code IS NULL THEN
            RETURN QUERY SELECT FALSE, NULL::UUID, NULL::UUID, 'Código de invitación requerido'::TEXT;
            RETURN;
        END IF;

        SELECT id INTO v_empresa_id
        FROM core_empresas
        WHERE ruc = p_invite_code AND estado = 'activo' AND deleted_at IS NULL
        LIMIT 1;

        IF v_empresa_id IS NULL THEN
            RETURN QUERY SELECT FALSE, NULL::UUID, NULL::UUID, 'Código de invitación no válido'::TEXT;
            RETURN;
        END IF;
    ELSE
        RETURN QUERY SELECT FALSE, NULL::UUID, NULL::UUID, 'Modo inválido'::TEXT;
        RETURN;
    END IF;
    ...
    RETURN QUERY SELECT TRUE, v_empresa_id, v_usuario_id, 'Registro exitoso'::TEXT;
...
BEGIN
    ...
    IF p_nombre IS NULL OR p_nombre = '' THEN
        RETURN QUERY SELECT FALSE, NULL::UUID, 'El nombre es obligatorio'::TEXT;
        RETURN;
    END IF;

    IF p_tipo NOT IN ('circulo', 'poligono') THEN
        RETURN QUERY SELECT FALSE, NULL::UUID, 'Tipo de geocerca inválido'::TEXT;
        RETURN;
    END IF;
    ...
    IF p_id IS NOT NULL THEN
        ...
        IF v_geocerca_id IS NULL THEN
            RETURN QUERY SELECT FALSE, NULL::UUID, 'Geocerca no encontrada'::TEXT;
            RETURN;
        END IF;

        RETURN QUERY SELECT TRUE, v_geocerca_id, 'Geocerca actualizada correctamente'::TEXT;
        RETURN;
    END IF;

    IF p_empresa_id IS NULL THEN
        RETURN QUERY SELECT FALSE, NULL::UUID, 'empresa_id es obligatorio para crear'::TEXT;
        RETURN;
    END IF;
    ...
    RETURN QUERY SELECT TRUE, v_geocerca_id, 'Geocerca creada correctamente'::TEXT;
...
-- ============================================================================
-- TRIGGERS AUTO-GENERACIÓN
-- ============================================================================

-- Auto-generar checkpoints (compatibilidad)
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM operations_checkpoints WHERE viaje_id = NEW.id AND deleted_at IS NULL) THEN
        RETURN NEW;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM operations_paradas WHERE ruta_id = NEW.ruta_id AND deleted_at IS NULL) THEN
        RETURN NEW;
    END IF;
...
-- Auto-generar visitas+operaciones
...
-- Vista: última posición GPS
CREATE VIEW v_ultima_posicion_gps
WITH (security_invoker = true)
AS
SELECT
    vehiculo_id, empresa_id, viaje_id, conductor_id,
...
COMMENT ON VIEW v_kpi_first_attempt IS
    'FADR con dos denominadores explícitos: among_successful y among_attempted.';
...
CREATE POLICY "Usuarios SELECT propia" ON core_usuarios
    FOR SELECT USING (empresa_id = public.user_empresa_id() OR auth_user_id = auth.uid());
CREATE POLICY "Usuarios UPDATE propia" ON core_usuarios
    FOR UPDATE USING (empresa_id = public.user_empresa_id() OR auth_user_id = auth.uid())
...
CREATE POLICY "Usuarios DELETE propia" ON core_usuarios
    FOR DELETE USING (empresa_id = public.user_empresa_id() OR auth_user_id = auth.uid());

-- core_roles (catálogo legible para todos)
...
CREATE POLICY "Sucursales de la empresa" ON core_sucursales
    FOR ALL USING (empresa_id = public.user_empresa_id()) WITH CHECK (empresa_id = public.user_empresa_id());
...
-- Tablas accedidas a través de relación padre
...
CREATE POLICY "Checklists Items de la empresa" ON fleet_checklists_items
    FOR ALL USING (checklist_id IN (SELECT id FROM fleet_checklists WHERE empresa_id = public.user_empresa_id()));
CREATE POLICY "Checklists Plantillas de la empresa" ON fleet_checklists_plantillas
    FOR ALL USING (empresa_id = public.user_empresa_id());

-- Tablas accedidas a través de viaje
CREATE POLICY "Paradas de la empresa" ON operations_paradas
    FOR ALL USING (ruta_id IN (SELECT id FROM operations_rutas WHERE empresa_id = public.user_empresa_id()));
CREATE POLICY "Viajes-Conductores de la empresa" ON operations_viajes_conductores
    FOR ALL USING (viaje_id IN (SELECT id FROM operations_viajes WHERE empresa_id = public.user_empresa_id()));
...
CREATE POLICY "Mensajes de la empresa" ON communication_chat_mensajes
    FOR ALL USING (chat_id IN (SELECT id FROM communication_chats WHERE empresa_id = public.user_empresa_id()));

-- Tablas accedidas a través de paquete/carga
CREATE POLICY "Paquetes-cargas de la empresa" ON shipping_paquetes_cargas
    FOR ALL USING (carga_id IN (SELECT id FROM shipping_cargas WHERE empresa_id = public.user_empresa_id()));
CREATE POLICY "Historial de la empresa" ON shipping_historial_estados
    FOR ALL USING (paquete_id IN (SELECT id FROM shipping_paquetes WHERE empresa_id = public.user_empresa_id()));

-- Catálogos globales con lectura pública
...
-- Motivos: lectura global + escritura por empresa
CREATE POLICY "Motivos visibles" ON operations_motivos
    FOR SELECT USING (empresa_id IS NULL OR empresa_id = public.user_empresa_id());
...
CREATE POLICY "Mis notificaciones" ON communication_notificaciones
    FOR ALL USING (usuario_id IN (SELECT id FROM core_usuarios WHERE auth_user_id = auth.uid()));
...
COMMENT ON TABLE operations_viaje_visitas IS 'Visita: parada ejecutable dentro de un viaje';
COMMENT ON TABLE operations_visita_operaciones IS 'Operación: QUÉ debe hacerse con un paquete en una visita';
COMMENT ON TABLE operations_operacion_ejecuciones IS 'Ejecución: cada intento real. Append-only tras finalizar';
COMMENT ON TABLE operations_custodia IS 'Cadena de custodia: quién tiene el paquete en cada momento';
COMMENT ON TABLE operations_evidencias IS 'Evidencias multimedia vinculadas a ejecuciones';
COMMENT ON TABLE integration_outbox IS 'Outbox pattern: eventos a publicar externamente';
COMMENT ON TABLE integration_inbox IS 'Inbox pattern: eventos recibidos externamente';
